...
| 組み込み評価器: エージェント向け | Intent Resolution, Task Adherence, Task Completion, Customer Satisfaction, Tool Call Accuracy, Tool Selection, Tool Input/Output 系, Task Navigation Efficiency, Quality Grader | Intent Resolution / Task Adherence / Task Completion / Customer Satisfaction / Quality Grader = プレビュー。Tool Call Accuracy / Tool Selection / Tool Input Accuracy / Tool Output Utilization / Tool Call Success / Task Navigation Efficiency = GA相当(プレビュー表記なし) | 対応 | 記載なし | 対応(evals API、`builtin.task_adherence` 等) | https://learn.microsoft.com/en-us/azure/foundry/concepts/evaluation-evaluators/agent-evaluators | **⚠ 使ってはいけない組合せが明記されている(2026-07-30 に正確な記述へ修正)**: 「**`tool_call_accuracy` / `tool input accuracy` / `tool_output_utilization` / `tool_call_success` / `groundedness` の各評価器は、エージェントの会話に次のツール呼び出しが含まれる場合は使うな**」— **Azure AI Search / Bing Grounding / Bing Custom Search / SharePoint Grounding / Code Interpreter / Fabric Data Agent / Web Search**。**完全サポートは File Search / Function Tool / MCP / Knowledge-based MCP の4つのみ。**`groundedness`(ツール系ではなく RAG 評価器)が対象に含まれる点に注意 |
