...
**さらにコンテナプロトコル 1.0.0 自体も非推奨**で、猶予期間後は 1.0.0 のエージェントへのリクエストがブロックされる(廃止日は未公表)。2.0.0 では `x-agent-foundry-call-id` の転送が必要になる代わりに、`x-agent-user-id` で **1 セッション内の複数ユーザー多重化**が安全に行える(1.0.0 では不可)。マルチテナントで hosted agent を使うなら 2.0.0 が前提。
