...
- ``FOUNDRY_PROJECT_ENDPOINT``(**省略可**): CLI(クライアント実行)には
  不要。hosted agent のデプロイ(hosting/deploy_hosted_agent.py)と Routine
  作成(scripts/setup_routine.py)だけが必要とするため、必須 3 点には含めず
  スクリプト側で存在を検証する
- ``HN_BRIEFING_AGENT_NAME``(既定 ``hn-briefing-agent``): hosted agent 名。
  デプロイ・Routine・hosted スモークで共有する
...
#: hosted agent の既定名(デプロイ・Routine・playground で参照する ID)
DEFAULT_AGENT_NAME = "hn-briefing-agent"
...
class FoundrySettings:
    ...
    #: プロジェクトエンドポイント(デプロイ/Routine スクリプトのみが使う。省略可)
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> FoundrySettings:
        ...
        return cls(
            ...
            agent_name=environ.get("HN_BRIEFING_AGENT_NAME", DEFAULT_AGENT_NAME),
        )
