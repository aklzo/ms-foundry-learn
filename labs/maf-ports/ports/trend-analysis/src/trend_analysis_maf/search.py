...
元アプリの ``DuckDuckGoTools`` の置き換え。実装は labs/agentic-search-maf の
``search.py`` から必要部分を移植(同一の HTML パーサ)。Foundry の Web search
ツール(Agent Service 組み込み)は DPA 対象外・別課金のため、本ポートでは
自前検索を既定とし、比較メモを README に記す。
"""
...
USER_AGENT = "trend-analysis-maf/0.1 (+https://github.com/aklzo/ms-foundry-learn)"
