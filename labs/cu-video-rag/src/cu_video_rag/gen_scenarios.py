...
ERR-1xx、毎月 n 日、n 件、n 日間)し、100 本規模でも回答値が動画間で衝突しないようにする。
一意性は corpus.validate() で機械検証する(コア動画の値との衝突も含む)。
...
FACT_KINDS = [
    ...
    ("limit", "{n2} 件", "一度に扱える上限: {n2} 件", "{app}で一度に処理できる件数の上限は?", "上限は {n2} 件(画面の注意書きに表示)。"),
...
def _fact(i: int) -> dict:
    ...
    # すべて動画インデックス i から単調に採番し、同じ kind の値が動画間で衝突しないようにする
    # (ラウンド 2 までは n5 = 3 + i % 5 などで「毎月 3 日」が 10 本に重複していた → 修正)
    # 値同士が部分文字列にならないようにも配慮する(「30 件」⊂「130 件」を避ける)
    vals = {"n5": 6 + i // 5, "n5000": 5000 + i, "n100": 100 + i, "n2": 15 + 9 * (i // 5), "n30": 10 + i}
    return {
        "kind": kind,
